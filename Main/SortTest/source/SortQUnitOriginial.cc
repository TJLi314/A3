...
int main () {
...

	{
		...
		MyDB_BufferManagerPtr myMgr = make_shared <MyDB_BufferManager> (400, 128, "tempFile");
...

	{
		// use the alternative iterator on the file
	        // load up the table supplier table from the catalog
                ...
                // there should be 16 records
                int counter = 0;
                while (myIter->advance ()) {
                        myIter->getCurrent (temp);
						// std::cout << temp->getBinarySize() << "FILE BINARY SIZE" << std::endl;
						// std::cout << temp << std::endl;
                        counter++;
                }
                QUNIT_IS_EQUAL (counter, 16);
...

	{
		...
		// now, go to the 12th page and iterate over it
		MyDB_RecordPtr temp = supplierTable.getEmptyRecord ();
		MyDB_RecordIteratorPtr myIter = supplierTable[12].getIterator (temp);
		...
		function <bool ()> myComp = buildRecordComparator (temp, temp2, "[suppkey]");
		MyDB_PageReaderWriterPtr sorted = supplierTable[12].sort (myComp, temp, temp2);
...

	{
		...
		function <bool ()> myComp = buildRecordComparator (rec1, rec2, "[suppkey]");

		// and sort
		system ("date");
		sort (2, supplierTable, outputTable, myComp, rec1, rec2);
		system ("date");
...

	{
		...
		otherSortedTable.loadFromTextFile ("supplierSmallSorted.tbl");
